...
class OdometryTricycleController: public GeomController<hardware_interface::JointStateInterface, UndercarriageGeom>
{
public:
    OdometryTricycleController() {}
    ...
    virtual bool init(hardware_interface::JointStateInterface* hw, ros::NodeHandle &root_nh, ros::NodeHandle& controller_nh){
        ...
        publish_timer_ = controller_nh.createTimer(ros::Duration(1/publish_rate), &OdometryTricycleController::publish, this);
        service_reset_ = controller_nh.advertiseService("reset_odometry", &OdometryTricycleController::srv_reset, this);
...
private:
    ...
    void calcDirect(PlatformState &state){
        ///'Introduction to Mobile Robot Control' by 'Spyros G Tzafestas' (chapter 2.3.3)
        double r_wheel = 0.0;
        double steer_angle = 0.0;
        double drive_vel = 0.0;
        double wheel_pos = 0.0;
        for (unsigned i=0; i<geom_->wheels_.size(); i++){
            if (!geom_->wheels_[i]->geom_.passive){
                r_wheel = geom_->wheels_[i]->geom_.dRadiusWheelMM / 1000.0;
                steer_angle = wheel_states_[i].dAngGearSteerRad;
                drive_vel = wheel_states_[i].dVelGearDriveRadS;
                wheel_pos = sqrt(pow(geom_->wheels_[i]->geom_.dWheelXPosMM,2) + pow(geom_->wheels_[i]->geom_.dWheelYPosMM,2)) / 1000.0;
                break;
            }
        }
        double v = r_wheel * cos(steer_angle) * drive_vel;
        double phi_dot = 2.0/3.0*wheel_pos * r_wheel * sin(steer_angle) * drive_vel;

        state.setVelX(v);
        state.dRotRobRadS = phi_dot;
    }
};

}

PLUGINLIB_EXPORT_CLASS( cob_omni_drive_controller::OdometryTricycleController, controller_interface::ControllerBase)
